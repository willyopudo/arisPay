...
@RestController
@RequestMapping("/api/v1/rejected-transactions")
@SecurityRequirement(name = "Bearer Authentication")
@CrossOrigin(origins = "http://localhost:3000")
public class TransactionRejectedController {

    private final TransactionRejectedServicePort transactionRejectedServicePort;
    ...
    private static final Logger logger = LogManager.getLogger(TransactionRejectedController.class);

    public TransactionRejectedController(JwtUtil jwtUtil, TransactionRejectedServicePort transactionRejectedServicePort, BankServicePort bankService, CompanyAccountServicePort<CompanyAccountDto> companyAccountService) {
        this.jwtUtil = jwtUtil;
        this.transactionRejectedServicePort = transactionRejectedServicePort;
    ...
    public ResponseEntity<TransactionDto> getTransactionByID(@PathVariable long id) {
        return ResponseEntity.ok(transactionRejectedServicePort.getTransactionById(id));
    }
    ...
    public ResponseEntity<Triplet<Page<TransactionDto>, List<List<SelectDto>>, ISummary>> getAllTransactions(@RequestParam(defaultValue = "0") int page,
        ...
        ISummary transactionSummary = transactionRejectedServicePort.getTransactionRejectedSummaries(companyId).orElse(null);

        Pageable pageable = PageRequest.of(page-1, itemsPerPage);
        return ResponseEntity.ok(new Triplet<> (transactionRejectedServicePort.getTransactions(companyId, pageable, filterDto), selectOptions, transactionSummary));
    }
